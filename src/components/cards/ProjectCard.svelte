<script>
	...
	const toggleDetails = () => (detailsRevealed = !detailsRevealed);
</script>
...
<section>
			...
			<div class="container">
				<div class="row center">
					<btn on:click={() => toggleDetails()}
						><img class="circle responsive-img project-icon" src={img} /></btn
					>
				</div>
				<btn on:click={() => toggleDetails()}>
	...
	{:else}
		<div in:fade class="card details-revealed">
			<div class="container">
				<btn class="col s1 offset-s11" on:click={() => toggleDetails()}
					><i class="material-icons green-text text-darken-3">close</i></btn
				>
				<btn on:click={() => toggleDetails()}
					><span class="card-title grey-text text-darken-4">{name}</span></btn
				>
				<div class="row">
